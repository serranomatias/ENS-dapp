...
export default function Home() {
  ...
  /**
   * A `Provider` is needed to interact with the blockchain - reading transaction,
   ...
   * Metamask exposes a Singer API to allow your website to request signatures from the
   * user using Signer functions.
   */
 ...
 /*
  renderButton: Returns a button based on the state of the drop
 */
  ...
  // The array at the end of function call represents what state changes will
  // trigger this effect
  // In this case, whenever the value of `walletConnected` changes - this effect
  // will be called
  useEffect(() => {
    // if wallet is not connected, create a new instance of Web3Modal and connect the
    // Metamask wallet
    if(!walletConnected) {
      // Assing the Web3Modal class to the reference object by setting it's `current` value
